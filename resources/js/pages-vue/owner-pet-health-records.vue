    ...
    <div class="flex flex-col gap-6">
      ...
      <article class="rounded-3xl border border-slate-100 bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.015)] relative overflow-hidden flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            <h2 class="text-xl font-extrabold text-slate-800">Hồ sơ bệnh án của {{ petNameDisplay }}</h2>
            <div class="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-xs text-slate-400">
              <p><span class="font-semibold text-slate-500">Loài:</span> {{ translateSpecies(pet?.species?.name) }}</p>
              <p v-if="pet?.breed"><span class="font-semibold text-slate-500">Giống:</span> {{ pet?.breed }}</p>
              <p v-if="pet?.weight"><span class="font-semibold text-slate-500">Cân nặng:</span> {{ pet?.weight }} kg</p>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <!-- Back button -->
          <a href="/owner/pets" class="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-extrabold text-slate-600 shadow-sm transition hover:border-indigo-500 hover:text-indigo-600 gap-1.5">
      ...
      <div v-if="activeTab === 'records'" class="space-y-4">
        ...
        >
          ...
          <div v-show="expandedRecordIds.includes(record.id)" class="px-6 pb-6 border-t border-slate-50 pt-5 space-y-6">
            ...
            <!-- Treatment Protocol, Follow up plan & Disease progress -->
...
import { ArrowLeft, FileText, PawPrint, Activity, ChevronDown, ChevronUp, Thermometer, Scale, Heart, Pill, FileSpreadsheet, CheckCircle2, AlertCircle } from '@lucide/vue';
...
async function loadData() {
    ...
    try {
        ...
        // Expand the first medical record by default
