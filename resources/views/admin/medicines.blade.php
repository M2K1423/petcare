<x-layout.app
    title="Quản lý thuốc | {{ config('app.name', 'PetCare') }}"
    ...
    <section class="grid gap-6 xl:grid-cols-[1.4fr_0.95fr]">
        <article class="rounded-3xl border border-[#DDE1E6] bg-[#FFFFFF] p-6 shadow-[0_16px_36px_rgba(0,0,0,0.05)]">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-[#333333]">Danh mục thuốc</h1>
                    <p class="mt-2 text-sm text-[#4A4A4A]">Quản lý sản phẩm, giá bán, số lượng tồn kho và thông tin hết hạn.</p>
                </div>
                <div class="rounded-2xl border border-[#D7E6F5] bg-[#F5FAFF] px-4 py-3 text-right">
                    <p class="text-xs uppercase tracking-[0.14em] text-[#5078A0]">Trạng thái tồn kho</p>
                    <p id="admin-medicine-summary" class="mt-1 text-sm font-semibold text-[#2A6496]">Đang tải...</p>
                </div>
            </div>
            ...
            <section class="mt-6 grid gap-4 md:grid-cols-3">
                <div class="rounded-2xl border border-[#F5D0A9] bg-[#FFF7ED] px-4 py-4">
                    <p class="text-xs font-semibold uppercase tracking-[0.14em] text-[#C2410C]">Sắp hết hàng</p>
                    <p id="admin-low-stock-count" class="mt-2 text-2xl font-extrabold text-[#9A3412]">0</p>
                    <p class="mt-1 text-xs text-[#9A3412]">Sản phẩm có số lượng tồn kho từ 5 trở xuống.</p>
                </div>
                <div class="rounded-2xl border border-[#FECACA] bg-[#FEF2F2] px-4 py-4">
                    <p class="text-xs font-semibold uppercase tracking-[0.14em] text-[#B91C1C]">Đã hết hạn</p>
                    <p id="admin-expired-count" class="mt-2 text-2xl font-extrabold text-[#991B1B]">0</p>
                    <p class="mt-1 text-xs text-[#991B1B]">Sản phẩm không còn được phép bán.</p>
                </div>
                <div class="rounded-2xl border border-[#FDE68A] bg-[#FFFBEB] px-4 py-4">
                    <p class="text-xs font-semibold uppercase tracking-[0.14em] text-[#B45309]">Sắp hết hạn</p>
                    <p id="admin-expiring-count" class="mt-2 text-2xl font-extrabold text-[#92400E]">0</p>
                    <p class="mt-1 text-xs text-[#92400E]">Sản phẩm hết hạn trong 30 ngày tới.</p>
                </div>
            </section>

            <section class="mt-6 grid gap-4 lg:grid-cols-[1.3fr_0.7fr_0.7fr]">
                <div>
                    <label for="admin-medicine-search" class="text-sm font-semibold text-[#333333]">Tìm kiếm</label>
                    <input id="admin-medicine-search" type="text" placeholder="Tìm theo tên, SKU, đơn vị, mô tả..." class="mt-2 w-full rounded-2xl border border-[#C1C4C9] bg-white px-4 py-3 text-sm text-[#333333] outline-none transition focus:border-[#2A6496]">
                </div>
                <div>
                    <label for="admin-stock-filter" class="text-sm font-semibold text-[#333333]">Lọc tồn kho</label>
                    <select id="admin-stock-filter" class="mt-2 w-full rounded-2xl border border-[#C1C4C9] bg-white px-4 py-3 text-sm text-[#333333] outline-none transition focus:border-[#2A6496]">
                        ...
                        <option value="low">Chỉ hàng sắp hết</option>
                        <option value="out">Chỉ hàng hết kho</option>
                        <option value="healthy">Chỉ hàng còn tốt</option>
                    </select>
                </div>
                <div>
                    <label for="admin-expiry-filter" class="text-sm font-semibold text-[#333333]">Lọc hạn dùng</label>
                    <select id="admin-expiry-filter" class="mt-2 w-full rounded-2xl border border-[#C1C4C9] bg-white px-4 py-3 text-sm text-[#333333] outline-none transition focus:border-[#2A6496]">
                        <option value="all">Tất cả trạng thái hạn dùng</option>
                        <option value="expired">Chỉ hàng hết hạn</option>
                        <option value="expiring">Sắp hết hạn</option>
                        <option value="safe">Còn hạn an toàn</option>
                        <option value="missing">Không có ngày hết hạn</option>
                    </select>
                </div>
            </section>

            <div class="mt-4 flex items-center justify-between gap-4">
                <p id="admin-filter-result" class="text-sm text-[#4A4A4A]">Đang hiển thị tất cả thuốc.</p>
        ...
        <article class="rounded-3xl border border-[#DDE1E6] bg-[#FFFFFF] p-6 shadow-[0_16px_36px_rgba(0,0,0,0.05)]">
            <div class="flex items-center justify-between gap-3">
                <h2 id="admin-medicine-form-title" class="text-lg font-bold text-[#333333]">Thêm thuốc</h2>
            ...
            <form id="admin-medicine-form" class="mt-5 space-y-4">
                ...
                <div>
                    <label for="medicine-name" class="text-sm font-semibold text-[#333333]">Tên</label>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        <label for="medicine-category" class="text-sm font-semibold text-[#333333]">Danh mục</label>
                        <input id="medicine-category" name="category" type="text" placeholder="Chăm sóc tiêu hóa, chăm sóc da..." class="mt-2 w-full rounded-2xl border border-[#C1C4C9] bg-white px-4 py-3 text-sm text-[#333333] outline-none transition focus:border-[#2A6496]">
                    </div>
                    <div>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        <label for="medicine-unit" class="text-sm font-semibold text-[#333333]">Đơn vị</label>
                        <input id="medicine-unit" name="unit" type="text" placeholder="hộp, chai..." class="mt-2 w-full rounded-2xl border border-[#C1C4C9] bg-white px-4 py-3 text-sm text-[#333333] outline-none transition focus:border-[#2A6496]">
                    </div>
                    <div>
                        <label for="medicine-image-url" class="text-sm font-semibold text-[#333333]">URL hình ảnh</label>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        <label for="medicine-price" class="text-sm font-semibold text-[#333333]">Giá (VND)</label>
                    ...
                    <div>
                        <label for="medicine-stock" class="text-sm font-semibold text-[#333333]">Số lượng tồn</label>
                ...
                <div>
                    <label for="medicine-expiration-date" class="text-sm font-semibold text-[#333333]">Ngày hết hạn</label>
                ...
                <div>
                    <label for="medicine-description" class="text-sm font-semibold text-[#333333]">Mô tả</label>
